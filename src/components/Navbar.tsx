    ...
    <header className="w-full z-50">
      <nav className={`w-full fixed top-0 transition-all duration-300 px-6 sm:px-12 ${scrolled ? 'py-4 shadow-sm' : 'py-5'} ${navBg}`}>
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          {/* Desktop nav */}
          <div className="hidden lg:flex landscape-nav items-center gap-8">
            {[
          ...
          {/* Desktop CTAs */}
          <div className="hidden lg:flex landscape-nav items-center gap-3">
          ...
          <button onClick={() => setIsOpen(!isOpen)}
            className="lg:hidden landscape-toggle p-2 cursor-pointer transition text-[#1a1625]">
            {isOpen ? <X className="w-5 h-5" /> : <Menu className="w-5 h-5" />}
          </button>
        </div>

        {/* Mobile menu — always mounted, GPU-accelerated animation */}
        <div className={`lg:hidden landscape-menu absolute top-full left-0 right-0 bg-white border-b border-[#e5e0ec] shadow-sm z-50 transition-all duration-300 ease-out ${isOpen ? 'opacity-100 translate-y-0 pointer-events-auto visible' : 'opacity-0 -translate-y-2 pointer-events-none invisible'}`}>
